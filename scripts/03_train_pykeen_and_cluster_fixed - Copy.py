...
def main():
    ...
    if len(patient_ids) < 2:
        print(f" Found only {len(patient_ids)} patient entities using prefix '{args.patient_prefix}'.")
    ...
    try:
        ...
        print(f" Could not write Excel (missing openpyxl?). Saved CSV only. Details: {e}")
        print(f" Saved table:\n - {csv_path}")
